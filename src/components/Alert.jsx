...
const Alert = ({ status, alertMsg }) => {
  return (
    ...
    >
      ...
      {status === "Warning" && (
        <div className="px-4 py-2 rounded-md bg-yellow-400 shadow-md shadow-emerald-500 ">
          <p className="text-lg text-primary">{alertMsg}</p>
        </div>
      )}
      {status === "Danger" && (
        <div className="px-4 py-2 rounded-md bg-red-400 shadow-md shadow-emerald-500 ">
          <p className="text-lg text-primary">{alertMsg}</p>
        </div>
      )}
    </motion.div>
